...
const RegisterProductScreen = () => {
	...
	console.log(categoryId, 'id de category')
	...
	const getCategories = () => {
		categoryServices
			.getCategories()
			.then((data) => {
				console.log(data, 'es data de categories')
	...
	const handleFile = (e) => {
		...
		uploadServices
			...
			.then((res) => {
				console.log(res, 'respuesta del backend')
				setIsLoading(false)
			...
			.catch((error) =>
				alert('Error,  el tamaño de la imagén es muy grande, intentalo de nuevo')
			)
	}
	const getColor = (res) => {
		setCheckColor(true)
											...
											images?.map((item) => {
												...
												return (
													<div className="item-minigalery" key={item._id}>
														<img src={`http://localhost:5005/${item.url}`} alt={item} />
														<div
								...
								<div className="container-uploaded-images">
									...
									{image && (
										<div className="image-root-register">
											<img
												src={`http://localhost:5005/${images[images.length - 1]?.url}`}
												alt={`http://localhost:5005/${images[0]?.url}`}
											/>
										</div>
									)}
									{/* <div className='mini-galery' >
